                      ...
                      SizedBox(
                        ...
                        child: Row(
                          children: [
                            ...
                            const Spacer(),
                            FilledButton(
                              onPressed: (c.lastScanSuccess.value ||
                                      c.scannedTags.isNotEmpty)
                                  ? () async {
                                      final ctrl =
                                          Get.find<MarkingController>();
                                      final result =
                                          await ctrl.finishMarkingItem(
                                        widget.item.id,
                                        markingId: widget.item.markingId,
                                        taskId: widget.item.taskId,
                                      );
                                      if (!result.ok) {
                                        await AppDialog.showError(
                                          ...
                                          reason:
                                              'Сервер вернул ошибку или нет связи.',
                                          solution:
                                      ...
                                      // Останавливаем сканирование, если ещё идёт
                                      if (_isReading.value) {
                                        try {
                                          final rfid =
                                              Get.find<RfidController>();
                                          await rfid.stop();
                                        } catch (_) {}
                                        _isReading.value = false;
                                      }
                                      ...
                                      AppSnackbar.showSuccess('Успешно',
                                  ...
                                  : null,
                              style: FilledButton.styleFrom(
                                backgroundColor: (c.lastScanSuccess.value ||
                                        c.scannedTags.isNotEmpty)
                                    ? Colors.black
                                    : Colors.grey,
                              ),
